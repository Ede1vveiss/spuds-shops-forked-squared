...
public class ShopScreenHandlerCustomer extends AbstractContainerMenu {
    ...
    public ShopScreenHandlerCustomer(int syncId, Inventory playerInv, RegistryFriendlyByteBuf buf) {// clientInit
    ...
    public ShopScreenHandlerCustomer(int syncId, Inventory playerInv, AbstractShopEntity shop,
            AbstractShopEntity.InventoryDelegate inventoryDelegate) {// serverInit
        super(ModScreenHandlers.SHOP_SCREEN_HANDLER_CUSTOMER.get(), syncId);
        this.shopInventory = shop.getInventoryDelegate(playerInv.player);
        this.pos = shop.getBlockPos();
        this.SCREEN_SETTINGS = null;
        this.playerInventory = playerInv;
        var iItemHandler = shop.getLevel().getCapability(Capabilities.ItemHandler.BLOCK, shop.getBlockPos(), null);
    ...
    class shop_vendor_slot extends Slot {
        ...
        public shop_vendor_slot(AbstractShopEntity.InventoryDelegate inventory, int index, int x, int y,
                ShopScreenHandlerCustomer handler) {
            super(inventory, index, x, y);
